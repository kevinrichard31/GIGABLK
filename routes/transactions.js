...
const routeHelpers = require("./informations.js");
...
app.post("/addToPool", async (req, res) => {
  ...
  switch (req.body.message.type) {
    ...
    case "sendToken":
      ...
      console.log("🌱 - file: router.js:121 - app.post - wallet:", wallet)
      if (wallet && wallet.tokens && wallet.tokens.GIGATREE) {
        if(wallet.tokens.GIGATREE.value <= 0){
          res.json("Error, not enough Gaz Fees")
          return;
        }
      }      
      ...
      if (wallet && wallet.tokens.GIGATREE) {
        if (wallet.tokens.GIGATREE.value >= gazFees) {
